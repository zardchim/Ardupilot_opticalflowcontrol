...
#define IsMOUSE        // are we using mouse sensor for position hold?  use this OR GPS not both!
...
void loop()
{
  ...
  if ((currentTime-mainLoop) > 5)    // about 200Hz (every 5ms)
  {
    ...
    // read mouse data if available
#ifdef IsMOUSE
    if( Serial3.available() ) {
        mouse_addChar(Serial3.read());
    }
#endif
    ...
    if(flightMode == STABLE_MODE) {    // STABLE Mode
      ...
      if (AP_mode == AP_NORMAL_MODE) {   // Normal mode
...
#if AIRFRAME == QUAD
      #ifdef IsMOUSE  // use mouse for position control if we have it
        Attitude_control_v3(command_rx_roll+command_mouse_roll+command_RF_roll,command_rx_pitch+command_mouse_pitch+command_RF_pitch,command_rx_yaw);
      #else
        ...
        if( altitude_control_method == ALTITUDE_CONTROL_NONE ) {
            ...
            heli_attitude_control(command_rx_roll+command_gps_roll,command_rx_pitch+command_gps_pitch,(ch_throttle_altitude_hold-1000)/10,command_rx_yaw);
            //SerPri("alt hold: ");
            //SerPrln((ch_throttle_altitude_hold-1000)/10);
        }
#endif
      }
    }
  ...
  if(!SW_DIP2) camera_output();
    ...
    if (AP_mode == AP_AUTOMATIC_MODE)
    {
      ...
      if( mouse_new_data ) {
          ...
          mouse_new_data = 0;  // mark that we've consumed the data
      }
      #endif
      ...
      #ifdef IsMOUSE
      mouse_reset_I_terms_navigation();
      #endif
    } // if (AP_mode == AP_AUTOMATIC_MODE)
  }  
